...
class QuizActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val q = bundle?.get("quiz")

        if (q == 1){
            questionList = setQuestionData.getQuestionKubus()
            quiz(questionList!!, q as Int)
        } else if (q == 2){
            questionList = setQuestionData.getQuestionBalok()
            quiz(questionList!!, q as Int)
        } else if (q == 3){
            questionList = setQuestionData.getQuestionLimas()
            quiz(questionList!!, q as Int)
        } else if (q == 4) {
            questionList = setQuestionData.getQuestionLingkaran()
            quiz(questionList!!, q as Int)
        }
    }
    ...
    fun quiz(questionList: ArrayList<QuestionData>, q: Int){
        setQuestion()
            ...
            if (selectedOption != 0) {
                ...
                when {
                    currentPosition <= questionList!!.size -> {
                        ...
                        intent.putExtra(setQuestionData.score, score.toString())
                        intent.putExtra("TotalSize", questionList!!.size.toString())
                        intent.putExtra("modul", q)
